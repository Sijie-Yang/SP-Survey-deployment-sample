...
// Function to get Supabase configuration from localStorage or environment
function getSupabaseConfig() {
  ...
  try {
    ...
    console.log('localStorage raw:', savedConfig);
    ...
    if (savedConfig) {
      ...
      if (parsed.enabled && parsed.url && (parsed.secretKey || parsed.anonKey)) {
        ...
        return { url: parsed.url, key: parsed.secretKey || parsed.anonKey, source: 'localStorage' };
      } else {
        console.log('localStorage config incomplete or disabled');
      }
    } else {
...
export async function saveSurveyResponse(completeData) {
  try {
    if (!supabase) {
      ...
      console.log('Survey response saved to localStorage:', responseData)
      return { success: true, data: responseData, storage: 'localStorage' }
    }
